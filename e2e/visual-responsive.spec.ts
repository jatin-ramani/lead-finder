...
async function mockProduct(page: Page) {
  await page.route("**/*", async (route) => {
    ...
    const apiPath = apiPathForMockRequest(request.url());
    if (!apiPath) return route.continue();
    if (apiPath === "/auth/me") return route.fulfill({ json: { authenticated: request.headers().cookie?.includes("leadfinder_session=") ?? false } });
    ...
    if (apiPath in responses) return route.fulfill({ json: responses[apiPath] });
...
test("all product routes are responsive without horizontal overflow", async ({ context, page }, testInfo) => {
  ...
  for (const route of ["/", "/businesses", "/scanner", "/scraping", "/system"]) {
    ...
    await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth), { message: `No horizontal overflow on ${route}` }).toBe(true);
