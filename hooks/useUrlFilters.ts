...
const FILTER_PARAMS = ["search", "city", "category", "has_website", "has_email", "has_phone"] as const;
...
export interface UrlFilters {
  ...
  search: string; city: string; category: string;
...
export function useUrlFilters(): UrlFilters {
  ...
  const page = readInt(params.get("page"), 1); const pageSize = readInt(params.get("pageSize"), DEFAULT_PAGE_SIZE);
  ...
  const setPage = useCallback((next: number, size?: number) => apply({ page: next, ...(size === undefined ? {} : { pageSize: size }) }), [apply]);
  const setSort = useCallback((field: BusinessSortField, order: SortOrder) => apply({ sortBy: field, sortOrder: order, page: 1 }), [apply]);
  const resetFilters = useCallback(() => apply({ search: undefined, city: undefined, category: undefined, has_website: undefined, has_email: undefined, has_phone: undefined, page: 1 }), [apply]);
  const activeCount = FILTER_PARAMS.filter((key) => params.has(key)).length;
  const query = useMemo(() => ({ page, pageSize, sortBy, sortOrder, ...(search && { search }), ...(city && { city }), ...(category && { category }), ...(hasWebsite !== undefined && { has_website: hasWebsite }), ...(hasEmail && { has_email: true }), ...(hasPhone && { has_phone: true }) }), [page,pageSize,sortBy,sortOrder,search,city,category,hasWebsite,hasEmail,hasPhone]);
  return { query, search, city, category, hasWebsite, hasEmail, hasPhone, page, pageSize, sortBy, sortOrder, activeCount, hasActiveFilters: activeCount > 0, setFilter, setPage, setSort, resetFilters };
}
