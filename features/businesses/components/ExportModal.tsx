...
function filtersOf(query: BusinessQuery) {
  const { search, city, category, has_website, has_email, has_phone, is_favorite, lead_status, lead_grade, min_lead_score, max_lead_score, tags } = query;
  return { search, city, category, has_website, has_email, has_phone, is_favorite, lead_status, lead_grade, min_lead_score, max_lead_score, tags };
}
...
export default function ExportModal({ open, ...props }: Props) { return <Modal title="Export businesses" open={open} onCancel={props.onClose} footer={null} destroyOnHidden className="lf-modal">{open && <Body {...props} />}</Modal>; }
